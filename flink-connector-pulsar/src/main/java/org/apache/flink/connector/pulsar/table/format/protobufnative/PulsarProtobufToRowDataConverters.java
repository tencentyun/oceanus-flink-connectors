...
/** Convert protobuf data to a flink row data. */
public class PulsarProtobufToRowDataConverters {
    ...
    public static ProtobufToRowDataConverter createRowConverter(RowType rowType) {
        final ProtobufToRowDataConverter[] fieldConverters =
                rowType.getFields().stream()
                        .map(RowType.RowField::getType)
                        .map(PulsarProtobufToRowDataConverters::createNullableConverter)
                        .toArray(ProtobufToRowDataConverter[]::new);
            ...
            for (int i = 0; i < arity; ++i) {
                ...
                row.setField(
                        ...
                        fieldConverters[i].convert(
                                record.getField(
                                        record.getDescriptorForType().findFieldByName(fieldName))));
            }
            return row;
        };
    }
    ...
    private static ProtobufToRowDataConverter createNullableConverter(LogicalType type) {
        final ProtobufToRowDataConverter converter = createConverter(type);
        return object -> {
            if (object == null) {
                return null;
            }
    ...
    private static ProtobufToRowDataConverter createArrayConverter(ArrayType arrayType) {
        final ProtobufToRowDataConverter elementConverter =
    ...
    private static ProtobufToRowDataConverter createMapConverter(MapType type) {
        ...
        final ProtobufToRowDataConverter keyConverter = createNullableConverter(type.getKeyType());
